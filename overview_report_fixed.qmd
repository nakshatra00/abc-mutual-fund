...
# Store key metrics for inline display
total_funds = len(summary_stats['funds_analyzed'])
total_value_display = f"{summary_stats['total_value_lacs']:,.0f}"
total_holdings = summary_stats['total_holdings']
overall_yield_display = f"{summary_stats['overall_weighted_yield']:.2f}%"
analysis_date_display = summary_stats['analysis_date'][:10]
```

# Executive Summary

```{python}
print(f"This report provides a comprehensive analysis of **{total_funds}** mutual funds with a total portfolio value of **₹{total_value_display} Lacs** across **{total_holdings}** holdings.")
```

**Key Highlights:**

```{python}
print(f"- **Overall Weighted Average Yield:** {overall_yield_display}")
print(f"- **Analysis Date:** {analysis_date_display}")
print(f"- **Funds Analyzed:** {total_funds}")
```
...
print(overview_display.to_string(index=False))
```
...
rating_summary['Percentage'] = (overall_ratings['Market_Value_Lacs'] / overall_ratings['Market_Value_Lacs'].sum() * 100).apply(lambda x: f"{x:.1f}%")
rating_summary = rating_summary[['Rating', 'Holdings_Count', 'Market_Value_Lacs', 'Percentage']]
rating_summary.columns = ['Rating', 'Holdings', 'Market Value', 'Portfolio %']
...
print(issuer_display.to_string(index=False))
```
...
if existing_buckets:
    ...
    print("No yield bucket data available for visualization")
```
...
```{python}
print(f"1. **Yield Performance**: The overall portfolio weighted average yield is **{overall_yield_display}**")
```

2. **Credit Quality**: The majority of holdings maintain high credit ratings (AAA/AA category)

```{python}
print(f"3. **Diversification**: Portfolio spans across **{len(top_issuers)}** major issuers with varying concentration levels")
```

4. **Maturity Profile**: Where available, the portfolio shows balanced maturity distribution
...
```{python}
print(f"*Analysis covers {total_holdings} holdings across {total_funds} funds*")
```